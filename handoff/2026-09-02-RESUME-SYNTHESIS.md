...
### §11 addendum, 2026-09-03 01:45 UTC — degree-4 null at two primes; streamrank 2.08x

- Degree-4 loop-free family at n = 11: NON_MEMBER also at p = 1,000,033 on both sketches (3,514 vs 3,515). Two primes × two sketches now.
- streamrank overlap (bead fhw, CobaltHare, commit 883fcd4): generation overlapped with sketch/reduce; 20,000-column n=11 CUDA prefix 1,278 s → 615 s (2.08×) with byte-identical pivots (verified by the orchestrator from the committed pivot files). Gated binary SHA `36c0e3ce…` approved for new runs only. Generation itself remains the bottleneck; bead relu-depth-frontier-research-3ke targets colgen.
- F2 certificate: T1 verify11 OK (11,320 terms, 0 bad rows, 1,445 s); lattice PASS; referee verify run in progress. Ledger recording for the rung delivered by BoldFrog (bead 330, six commits to cacc60a); under independent audit before acceptance. One attribution error already seen: the orchestrator's T0 reviews (V-0009, V-0010) carry model_family "openai-gpt"; the orchestrator is Claude (Fable 5.1). To be corrected with the audit's findings.

### §11 addendum, 2026-09-03 02:00 UTC — second certificate (F2, forest pairs, 11,320 terms) verified at the same level
...
### §11 addendum, 2026-09-03 02:33 UTC — induction test confirmed on a second sketch; lemma REFEREED; ledger state

- ksi seed 2: rank 30,200 = augmented, MEMBER, pivot list identical to seed 1 (JSON SHA `79b5219a…`). The S_11-lift of the n = 10 certificate spans MAX_11 at two sketches, one prime; the n = 9 → 10 control of the same construction is running.
- Depth-2 realization lemma (revision 2b, `a1a70ba`): Opus review counts as T1 (same lineage as the author); GPT-lineage referee IndigoCarp: T2 PASS, 16/16 obligations, independent exact-Fraction construction on the upstream n = 6 certificate matched max on 30/30 points (report `a0265d2`). Ledger: C-0059@1 (lemma) computes REFEREED; C-0002@3 (MAX_11 ∈ ReLU_2) computes challenged / INDEPENDENTLY_REPLAYED with one open gap, G-0016 (84 hinge rows), assigned to IndigoCarp; G-0017 (verify11 multiply) and G-0018 (lemma T2) discharged.
- Ledger discipline: the audit repairs were made by editing records in place, so `verify-quick` now reports 22 SE-10 immutable-history findings (plus the pre-existing G-0015). Records must be corrected by superseding versions, not edits; a bounded repair bead follows, after which the ledger is frozen.

### §11 addendum, 2026-09-03 02:52 UTC — recursion holds at two consecutive rungs; 84-row gap explained; ledger history finding is permanent

- Induction test control (bead ksi): the same lift construction applied to the 337 upstream n = 9 terms gives a 114,814-orbit family at n = 10 that is MEMBER (rank 17,127; JSON SHA `a9f6adc6…`). Combined with the n = 11 result (two sketches, rank 30,200): cert(n) ⊆ span(lift(cert(n−1))) at 9 → 10 and 10 → 11. Prior update: P(inductive construction exists for all n) 0.20 → 0.30. Next: the exact lift on the n = 11 lift family (running on the A100) to obtain a certificate composed only of lifted n = 10 terms, then compare its coefficient structure with the n = 10 coefficients; the second prime for the lift family is running on the H100.
- G-0016 (84 hinge rows): confirmed from the data by IndigoCarp (commit 9565024): the lift's union covers all 21,222 pivot columns; the 84 rows outside the certificate's 169,166 are touched only by zero-coefficient pivots. Dischargeable.
- Ledger discipline: the deep-history walker compares every immutable record against every committed version, so the 22 SE-10 findings created by in-place edits cannot be cleared by any append-only repair; the only ways out (rewriting history, editing the verifier) are forbidden. Adjudication: leave them, record one deviation listing all 22 with commit hashes and cause, keep the corrected content, discharge G-0016, bind a T2 review to C-0002@3, then freeze the ledger. The final report will state the finding count plainly. Computed standings are unaffected.
- Loop-inclusive degree-4 family at n = 11 (bead sou): first sketch NON_MEMBER (8,667 vs 8,668 over 137,505 columns); second sketch and second prime running.

### §11 addendum, 2026-09-03 03:18 UTC — referee final: F2 T2 PASS; both certificates re-verified on the patched verifier; row-count gaps explained

- Referee (commit 28b8186, RESULT.md Parts I–III): F2 T2 PASS (byte-for-byte re-derivation of all 11,320 terms, acyclicity of every branch verified, independent exact recomputation OK, literal 20/20, lattice PASS, planted negative FAIL with predicted residuals). Both certificates re-run on the patched verify11 (binary `85af29b2…`): OK, exit 0, all 32 compared fields identical to the pre-fix runs.
- Row counts: run7 lift union 169,250 vs certificate 169,166 (84) and F2 146,176 vs 145,530 (646) are both the union over ALL pivot columns versus the support only; the extra rows are touched exclusively by zero-coefficient pivots (0 support incidences), audited from the problem files by IndigoCarp (run7) and the orchestrator (both). Quote the certificate figures (169,166 and 145,530) in any write-up.
- Process: `git commit -a`/`add -A` by concurrent agents mis-attributed the referee's F2 bundle (content intact); AGENTS.md now forbids blanket commits.

### §11 addendum, 2026-09-03 03:22 UTC — NOVELTY GATE FAILED: n ≤ 12 was published on 2026-09-01

- Verified by the orchestrator on the arXiv abstract page and the authors' repository: Rueß, Averkov, Brunck, Grillo, Hertrich, Loho, Stade, Stargalla, Sun, Winter, arXiv:2607.21651 **v2 (2026-09-01)**: "max_n is exactly representable with two hidden layers for every n ≤ 12" (rational weights; ⌈log_6(n/2)⌉+1 hidden layers in general; every CPWL function on R^d with d ≤ 11 in two hidden layers); certificates certificate_11_5.json (39,042 loop-inclusive terms) and certificate_12_5.json are public. Their Corollary 4.3 (already in v1) gives branch size ⌊(n−1)/2⌋ necessary. Their Section 5: n = 13 unresolved for lack of compute; the needed idea is "a structural principle relating solutions across arities".
- Consequences: C-0002@3 (our n = 11 certificate) is an independent replication with structurally different certificates (loop-free; forest-pair 11,320 terms), obtained two days after publication. The n = 12 arms and lift, the degree-4 tests (loop-free and loop-inclusive) are replications; stopped 03:20 UTC. The class-sum negative and the lifted-certificate recursion (two rungs) remain unstated in the literature; the recursion is exactly the direction their Section 5 names.
- Process failure, recorded plainly: the P2/P3 dated novelty gate was not re-run before the compute day; the 2026-09-02 sweep (memory "verified frontier facts") missed a v2 posted about eight hours earlier. Cost: roughly ten agent-hours of verification and ledger work on a replicated claim and about seventy dollars of GPU time. Search log: literature/novelty-search-2026-09-03.md (commit af8eedd); REF-0001 must be refreshed to v2.
- Live plan: everything now hinges on the weighted-lift recursion test (bead kbg). If coefficients factor at 9 → 10 and 10 → 11, attack n = 13 by lifting the published certificate_12_5 with streamed class sums (a tiny exact system instead of a ~100k-rank computation, which our method cannot do). If not, close the campaign with an honest Shape-2 report: replication, three negative structural results, an engine.

### §11 addendum, 2026-09-03 04:45 UTC — weighted-lift recursion test (bead kbg): NEGATIVE at 9 → 10 (exact)

- Hypotheses: H1 coefficient(t,e,f) = c_t · w_τ with τ an attachment type of the two new edges relative to the parent term; H2 = c_t · w_{t,τ}. Eight taxonomies from 1 to 455 classes (thirteen incidence features at the finest). Result: NON_MEMBER for every hypothesis and taxonomy, with exact rational duals; rank[S|b] = rank S + 1 in every case; replay and dual controls pass; re-executed by the orchestrator from the saved class sums. 10 → 11 accumulation in progress on the NVL.
- Reading: the lift family spans MAX_n (bead ksi) but no recursion whose new coefficients are a bounded local function of how the added edges attach, even with per-parent freedom, reproduces it. Together with the class-sum negative, any inductive construction must carry global isomorphism data of the extended signed graph. This closes the only route by which our machinery could have reached n = 13 (a tiny class-sum system instead of a ~100k-rank computation).
- Decision: no live route to the universal statement or to n = 13 with these methods. Recommend closing the campaign with an honest Shape-2 report: an independent replication of the published n ≤ 12 result with structurally different certificates, three exact negative structural results (class-sum, weighted-lift recursion at 9 → 10, degree-4 loop-free null — the last a replication of a published corollary), the engine, and the dated novelty failure.

### §11 addendum, 2026-09-03 05:10 UTC — recursion test negative at 10 → 11 as well (exact); all GPU boxes destroyed

- Bead kbg at 10 → 11 (parents = the 402 pinned n = 10 terms; 163,740 orbits; 243,122 rows; accumulated exactly): H1 NON_MEMBER at every taxonomy at both primes with exact duals (finest T3: rank 451 vs 452); H2 NON_MEMBER. Bead closed. The lifted family spans MAX_11 (bead ksi) but no locally-weighted recursion reproduces it at either rung.
- Compute: NVL instance destroyed 05:09 UTC; zero Vast instances remain. Swarm paused; ledger close-out parked. Campaign state: awaiting Duncan's decision on the Shape-2 close-out report.
- Scope of the kbg negative (agent's final report): H1 at all eight taxonomies at both rungs, and H2 at the base taxonomy T1 at both rungs. H2 at finer taxonomies (T2b: 6,419 unknowns; T2, T3 larger) is untested — an open cell with a stated retry predicate, not a negative. The 10 → 11 class-sum intermediate was on the destroyed NVL and would need ~1 h to regenerate.
