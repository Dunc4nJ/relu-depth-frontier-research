...
const STAGE_D_SOURCE_COMMIT_G0140: &str = "19107c5eed2cad00d48eff3dd9bea0c015ecce89";
...
const STAGE_D_SOURCE_AUDIT_SCHEMA_G0140: &str =
    "max11-g0162-g0140-stage-d-master-final4-source-audit-v1";
...
const STAGE_E_SOURCE_AUDIT_PREREG_PATH: &str =
    "artifacts/reviews/G-0163-g0140-stage-e-final4-source/PREREGISTRATION.md";
const STAGE_E_SOURCE_AUDIT_SCHEMA: &str = "max11-g0163-g0140-stage-e-final4-source-audit-v1";
...
const G0140_SCIENTIFIC_OUTPUT_PATHS: [&str; 5] = [
...
struct StageDSourceAuditChecks {
    ...
    stage_c_snapshot_digest_contract_verified: bool,
...
fn validate_protocol_manifest_output_boundary(protocol: &ManifestSnapshot) -> Result<()> {
    for output_path in G0140_SCIENTIFIC_OUTPUT_PATHS {
        ensure!(
            !protocol.bindings_by_path.contains_key(output_path),
            "G-0140 manifest circularly prebinds future scientific output: {output_path}"
        );
    }
    Ok(())
}
...
fn validate_stage_e_source_audit_gate(root: &Path, protocol: &ManifestSnapshot) -> Result<Binding> {
    ...
    git_commit_for_path(root, STAGE_E_SOURCE_AUDIT_PATH)?;
        ...
        "G-0163 subject commit does not match Stage-E source"
    );
    let preregistration = checked_repo_path(root, STAGE_E_SOURCE_AUDIT_PREREG_PATH)?;
        ...
        "G-0163 preregistration custody drift"
    );
    for (expected_path, binding) in stage_e_source_audit_bindings(&receipt) {
        ...
        git_commit_for_path(root, expected_path)?;
            ...
            "G-0140 manifest omits exact G-0163 subject binding: {expected_path}"
        );
    }
    let audit_binding = Binding {
        ...
        "G-0140 manifest omits exact G-0163 receipt binding"
    );
    ...
    git_is_ancestor(
        ...
        &receipt.preregistration.git_commit,
        "Stage-E source -> G-0163 preregistration",
    )?;
    git_is_ancestor(
        root,
        &receipt.preregistration.git_commit,
        &audit_commit,
        "G-0163 preregistration -> receipt",
    )?;
    git_is_ancestor(
        root,
        &audit_commit,
        &manifest_commit,
        "G-0163 receipt -> G-0140 manifest",
    )?;
    Ok(audit_binding)
}
...
fn validate_stage_d_source_audit_semantics(receipt: &StageDSourceAuditReceipt) -> Result<()> {
    let checks = &receipt.required_checks;
        ...
        receipt.schema == STAGE_D_SOURCE_AUDIT_SCHEMA_G0140
            ...
            && checks.stage_c_snapshot_digest_contract_verified
            ...
            && checks.producer_static_preflight_passed
            && checks.prohibited_scientific_modes_not_run,
        "G-0162 exact source-audit PASS contract drift"
    );
    ensure!(
        receipt.preregistration.path == STAGE_D_SOURCE_AUDIT_PREREG_PATH_G0140
            ...
            && receipt
                .preregistration
                .committed_and_pushed_before_runtime_checks,
        "G-0162 preregistration disclosure drift"
    );
    Ok(())
}
...
fn validate_stage_d_source_audit_gate(
    ...
    let preregistration = checked_repo_path(root, STAGE_D_SOURCE_AUDIT_PREREG_PATH_G0140)?;
        ...
        sha256_path(&preregistration)? == receipt.preregistration.sha256
            && git_commit_for_path(root, STAGE_D_SOURCE_AUDIT_PREREG_PATH_G0140)?
                == receipt.preregistration.git_commit,
        "G-0162 preregistration byte/Git custody drift"
    );
    for binding in [expected, &receipt.subject.bindings.master_source] {
            ...
            "G-0140 manifest omits exact G-0162 custody binding: {}",
            binding.path
        );
    }
    ...
    git_is_ancestor(
        ...
        &receipt.preregistration.git_commit,
        "Stage-D source -> G-0162 preregistration",
    )?;
    git_is_ancestor(
        root,
        &receipt.preregistration.git_commit,
        &audit_commit,
        "G-0162 preregistration -> receipt",
    )?;
    git_is_ancestor(
        root,
        &audit_commit,
        &manifest_commit,
        "G-0162 receipt -> G-0140 manifest",
    )?;
    Ok(expected.clone())
}
...
fn validate_g0140_manifest(root: &Path) -> Result<ManifestSnapshot> {
    ...
    validate_protocol_manifest_output_boundary(&snapshot)?;
    validate_source_audit(
        root,
        &snapshot,
        G0140_STAGE_A_SOURCE_AUDIT_PATH,
...
fn load_and_validate_stage_d_member(
    ...
    // Frozen sources and audits are manifest-bound. Scientific outputs cannot be:
    // their exact bindings are carried by downstream receipts and authenticated by
    // the committed manifest -> A -> B -> C -> D ancestry chain below.
    for candidate_binding in [&candidate.source_audit, &candidate.solver] {
        ensure!(
...
fn self_test() -> Result<()> {
    ...
    let empty_protocol = ManifestSnapshot {
        sha256: "0".repeat(64),
        bindings_by_path: BTreeMap::new(),
    };
    validate_protocol_manifest_output_boundary(&empty_protocol)?;
    for output_path in G0140_SCIENTIFIC_OUTPUT_PATHS {
        let mut circular_protocol = empty_protocol.clone();
        circular_protocol
            .bindings_by_path
            .insert(output_path.to_string(), "0".repeat(64));
        ensure!(
            validate_protocol_manifest_output_boundary(&circular_protocol).is_err(),
            "future scientific-output manifest cycle escaped: {output_path}"
        );
    }
    ...
    stage_d_unknown["unknown_extension"] = Value::Bool(true);
        ...
        "G-0162 exact typed source-audit hostile control escaped"
    );
