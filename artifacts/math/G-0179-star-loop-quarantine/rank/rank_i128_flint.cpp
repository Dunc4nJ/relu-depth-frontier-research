...
int main(int argc, char **argv) {
    if (argc != 10) {
        std::cerr << "usage: rank_i128_flint MATRIX ENCODING(i64le|i128le) INPUT_ROWS INPUT_COLS COL_START COL_END "
    ...
    try {
        ...
        if (selected_rows <= 0 || selected_rows != selected_cols) {
            throw std::runtime_error("selected matrix must be nonempty and square");
        }
        ...
        const auto determinant_start = Clock::now();
        const ulong determinant = nmod_mat_det(matrix);
        const auto determinant_end = Clock::now();
        ...
        Sha256 rref_sha;
        bool rref_identity = rank == selected_rows;
        for (slong i = 0; i < selected_rows; ++i) {
            for (slong j = 0; j < selected_cols; ++j) {
                const ulong value = nmod_mat_entry(matrix, i, j);
                sha_update_u64le(rref_sha, std::uint64_t(value));
                if (rref_identity && value != ulong(i == j)) rref_identity = false;
            }
        }
        const std::string rref_digest = rref_sha.finish();
        nmod_mat_clear(matrix);

        const bool full_rank = rank == selected_rows;
        if ((determinant != 0) != full_rank) {
            throw std::runtime_error("FLINT determinant/rank inconsistency");
        }
        if (full_rank && !rref_identity) throw std::runtime_error("full-rank square RREF is not identity");
        ...
        out << "],\n"
            ...
            << "  \"rank_mod_prime\": " << rank << ",\n"
            << "  \"full_rank_mod_prime\": " << (full_rank ? "true" : "false") << ",\n"
            << "  \"rref_identity\": " << (rref_identity ? "true" : "false") << ",\n"
        ...
        out << "],\n"
            ...
            << seconds(load_start, load_end) << ", \"determinant\": "
            << seconds(determinant_start, determinant_end) << ", \"rref\": "
        ...
        std::cerr << "rank_i128_flint: " << error.what() << '\n';
        return 1;
    }
}
