...
const MEMBER_RESULT_PATH: &str = "artifacts/math/G-0132/member_global_normal_form_replay_v1.json";
const MEMBER_MANIFEST_PATH: &str =
    "artifacts/math/G-0132/member_global_normal_form_manifest_v1.json";
const NONMEMBER_RESULT_PATH: &str = "artifacts/math/G-0132/full_degree5_separator_pricing_v1.json";
const SOURCE_AUDIT_RECEIPT_PATH: &str =
    "artifacts/reviews/G-0133-g0132-source/SOURCE_AUDIT_RECEIPT.json";
const SOURCE_AUDIT_REPORT_PATH: &str =
    "artifacts/reviews/G-0133-g0132-source/SOURCE_AUDIT_REPORT.md";
const SOURCE_AUDIT_SELF_TEST_PATH: &str =
    "artifacts/reviews/G-0133-g0132-source/SELF_TEST_RECEIPT.json";
const SOURCE_AUDIT_PROBE_SOURCE_PATH: &str =
    "artifacts/reviews/G-0133-g0132-source/independent_probe.py";
const SOURCE_AUDIT_PROBE_RECEIPT_PATH: &str =
    "artifacts/reviews/G-0133-g0132-source/INDEPENDENT_PROBE_RECEIPT.json";
...
const G0132_PREREGISTRATION_SHA256: &str =
    "73ccd2ce2a96c0d46b0a40166ca6a84050577cdba3f23ff12d1b89e043e8c692";
const G0132_PREREGISTRATION_COMMIT: &str = "6db51f8716d0ba0a82606bddc7573dafd889f2fe";
const G0133_PREREGISTRATION_SHA256: &str =
    "d2461477ce22c3f8afa036886b63988a3914303a54486bea4bb76d49d164b9bc";
...
const GLOBAL_CLAIM_BOUNDARY: &str = "Exact zero establishes only the frozen 132-term S_11-orbit functional identity through the pinned ordered-chamber normal-form uniqueness lemma; a residual refutes only this coefficient vector. Neither outcome proves family completeness, an unrestricted two-hidden-layer theorem, the all-n target, a lower bound, or a Lean theorem.";
...
const FROZEN_DIRECT_BINDINGS: &[(&str, &str, &str)] = &[
    ...
    (
        "g0132_preregistration",
        "artifacts/math/G-0132/PREREGISTRATION.md",
        G0132_PREREGISTRATION_SHA256,
    ),
    (
        "source_audit_preregistration",
        "artifacts/reviews/G-0133-g0132-source/PREREGISTRATION.md",
        G0133_PREREGISTRATION_SHA256,
    ),
    (
...
fn validate_compiled_and_static(root: &Path) -> Result<()> {
        ...
        sha256_bytes(COMPILED_SOURCE)
            == sha256_path(&checked_repo_path(
                root,
                "artifacts/math/G-0132/src/main.rs"
            )?)?,
        ...
        sha256_bytes(COMPILED_MANIFEST)
            == sha256_path(&checked_repo_path(
                root,
                "artifacts/math/G-0132/Cargo.toml"
            )?)?,
        ...
        sha256_bytes(COMPILED_LOCK)
            == sha256_path(&checked_repo_path(
                root,
                "artifacts/math/G-0132/Cargo.lock"
            )?)?,
        "running binary was compiled from different Cargo lockfile"
    );
    ensure!(
        sha256_bytes(COMPILED_PREREGISTRATION) == G0132_PREREGISTRATION_SHA256
...
fn validate_source_audit(
    ...
    let receipt: SourceAuditReceipt = serde_json::from_reader(BufReader::new(File::open(path)?))?;
        ...
        receipt.schema == "max11-g0133-g0132-source-audit-receipt-v1"
    ...
    for artifact in artifacts {
            ...
            artifact
                .path
                .starts_with("artifacts/reviews/G-0133-g0132-source/")
...
fn expected_manifest(root: &Path, transitive_inputs: Vec<Binding>) -> Result<StudyManifest> {
    ...
    let source = make_binding(root, "artifacts/math/G-0132/src/main.rs")?;
    let cargo_manifest = make_binding(root, "artifacts/math/G-0132/Cargo.toml")?;
    let cargo_lock = make_binding(root, "artifacts/math/G-0132/Cargo.lock")?;
    ...
    Ok(StudyManifest {
        schema: "max11-g0132-member-global-normal-form-manifest-v1".to_string(),
        selected_branch: "MEMBER".to_string(),
        output_path: MEMBER_RESULT_PATH.to_string(),
        preregistration_git_commit: G0132_PREREGISTRATION_COMMIT.to_string(),
        producer_git_commit: git_commit_for_path(root, "artifacts/math/G-0132/src/main.rs")?,
...
fn self_test() -> Result<()> {
    ...
    let temporary_directory = std::env::temp_dir().join(format!(
        "g0132-publish-self-test-{}-{unique}",
        std::process::id()
    ));
...
fn preflight(input_path: PathBuf, candidate_path: PathBuf) -> Result<()> {
    ...
    println!(
        "G-0132 preflight PASS: finite member admitted; {} terms; {} transitive inputs",
        candidate.terms.len(),
        transitive.len()
    );
    Ok(())
...
fn run(
    ...
    let result = if all_zero {
        "MEMBER_EXACT_GLOBAL_NORMAL_FORM_ZERO"
    } else {
        "MEMBER_EXACT_GLOBAL_NORMAL_FORM_RESIDUAL"
    };
    ...
    let output = Output {
        schema: "max11-g0132-member-global-normal-form-replay-v1",
        result,
        ...
        term_normal_form_transcript_sha256: sha256_bytes(&term_transcript),
...
fn main() -> Result<()> {
    ...
    if args.len() == 5 && args[1] == "--build-manifest" {
        ...
        args.len() == 5,
        "usage: g0132-member-global-normal-form --self-test | --preflight PANEL CANDIDATE | --build-manifest PANEL CANDIDATE MANIFEST | PANEL CANDIDATE MANIFEST OUTPUT"
    );
    run(
        PathBuf::from(&args[1]),
        PathBuf::from(&args[2]),
