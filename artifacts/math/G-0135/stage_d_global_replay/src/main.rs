...
const OUTPUT_PATH: &str = "artifacts/math/G-0135/new_member_global_replay_v1.json";
...
const PREREGISTRATION_PATH: &str = "artifacts/math/G-0135/PREREGISTRATION.md";
...
const OUTPUT_SCHEMA: &str = "max11-g0135-new-member-global-replay-v1";
const ZERO_RESULT: &str = "GLOBAL_EXACT_ZERO";
const RESIDUAL_RESULT: &str = "EXACT_RESIDUAL_BATCH_CONTINUE";
const DECISION_RULE: &str = "complete_arbitrary_precision_ordered_chamber_normal_form_aggregate";
const CLAIM_BOUNDARY: &str = "GLOBAL_EXACT_ZERO establishes only the complete exact ordered-chamber normal form of this frozen 163,740-family Stage-C member; EXACT_RESIDUAL_BATCH_CONTINUE refutes only that member and emits the deterministic next 32 rows. Neither branch proves family completeness, an unrestricted theorem, a lower bound, minimality, the all-n target, or a Lean theorem.";
...
struct Output {
    ...
    first_nonzero_linear: Option<ExactLinear>,
    next_batch_k: usize,
    next_selected_count: usize,
    next_selected_directions_i8_sha256: String,
    next_selected_exact_residuals_decimal_lf_sha256: String,
    next_selected: Vec<ExactHinge>,
...
fn validate_compiled_bytes(root: &Path) -> Result<()> {
    for (compiled, path) in [
        (COMPILED_SOURCE, STAGE_D_SOURCE_PATH),
        (COMPILED_ENGINE, STAGE_D_ENGINE_PATH),
        (COMPILED_MANIFEST, STAGE_D_CARGO_PATH),
        (COMPILED_LOCK, STAGE_D_LOCK_PATH),
...
fn validate_shared_manifest(root: &Path) -> Result<ManifestSnapshot> {
    ...
    let expected_outputs = BTreeMap::from([
        ...
        manifest.parameters.n == N
            ...
            && manifest.parameters.batch_k == BATCH_K
    ...
    for required in REQUIRED_MANIFEST_PATHS
        .iter()
        .copied()
        .chain(std::iter::once(executable.as_str()))
    {
        ensure!(
...
fn validate_stage_a_receipt(
    ...
    for (index, check) in receipt.carry_forward_checks.iter().enumerate() {
        ...
        receipt.batch_k == BATCH_K
            && receipt.selected_count == BATCH_K
            && receipt.selected.len() == BATCH_K
...
fn select_next_batch(
    ...
    let selected = aggregate
        ...
        "residual has fewer than 32 eligible hinges"
    );
    ensure!(
        selected
            .windows(2)
            .all(|pair| pair[0].direction < pair[1].direction),
        "next Batch32 order is not strict signed lexicographic"
    );
    for item in &selected {
        validate_direction(&item.direction)?;
        ensure!(
...
fn self_test() -> Result<()> {
    ...
    coefficient_mutant[0].coefficient =
        ...
        "Batch32 order/digest mutation control drift"
    );
    ...
    let temporary_directory = std::env::temp_dir().join(format!(
        "g0135-stage-d-publish-self-test-{}-{unique}",
        std::process::id()
    ));
...
fn static_preflight() -> Result<()> {
    ...
    println!("G-0135 Stage D outcome-blind static preflight PASS");
    Ok(())
}
...
fn full_preflight(manifest_path: &Path, candidate_path: &Path) -> Result<()> {
    ...
    println!(
        "G-0135 Stage D preflight PASS: {} selected; {} support; {} nonzero terms; 412-row member admitted; no scientific output written",
...
fn run(manifest_path: &Path, candidate_path: &Path, output_path: &Path) -> Result<()> {
    ...
    rayon::ThreadPoolBuilder::new()
        .num_threads(THREADS)
        .build_global()
        .context("build fixed Stage D thread pool")?;
    ...
    let selection_controls = if global_zero {
        ...
        "next Batch32 hostile control failed"
    );
    ...
    let output = Output {
        ...
        first_nonzero_hinge,
        first_nonzero_linear,
        next_batch_k: if global_zero { 0 } else { BATCH_K },
        next_selected_count: next_selected.len(),
        next_selected_directions_i8_sha256: next_direction_digest,
        next_selected_exact_residuals_decimal_lf_sha256: next_coefficient_digest,
        next_selected,
    ...
    let stdout = serde_json::json!({
        ...
        "next_selected_count": output.next_selected_count,
        "next_selected_directions_i8_sha256": output.next_selected_directions_i8_sha256,
        "next_selected_exact_residuals_decimal_lf_sha256": output.next_selected_exact_residuals_decimal_lf_sha256,
    });
    ...
    let end = load_and_validate_inputs(&root)?;
        ...
        "input/source/audit drift during Stage D replay"
    );
...
fn main() -> Result<()> {
    ...
    if args.len() == 2 && args[1] == "--self-test" {
        self_test()?;
        println!("G-0135 Stage D self-test PASS");
        return Ok(());
    }
    ...
    if args.len() == 4 && args[1] == "--preflight" {
        ...
        args.len() == 4,
        "usage: g0135-stage-d-global-replay --self-test | --preflight-static | --preflight MANIFEST STAGE_C_MEMBER | MANIFEST STAGE_C_MEMBER OUTPUT"
    );
    run(
        Path::new(&args[1]),
        Path::new(&args[2]),
