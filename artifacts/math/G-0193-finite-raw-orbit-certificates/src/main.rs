...
fn main() -> Result<()> {
    ...
    let raw_forms = input
        .records
        .iter()
        .map(|record| raw_form(record, 1))
        .collect::<Result<Vec<_>>>()?;

    let output = Output {
        schema: "g0193-g0109-raw-word-probe-v1",
        claim_boundary: "Exact raw permutation-word histograms for supplied finite records only.",
        rust_layout: Layout {
