#!/usr/bin/env python3
"""Fresh two-evaluator exact replay of the frozen G-0183 candidate identity."""
...
EXPECTED = {
    "candidate": "df295599ac4e0f3fc94666198d9aa075b081866efa8a7e5dcf43a67edb76220e",
...
class VerificationError(RuntimeError):
    """A frozen binding, candidate, or exact semantic gate failed."""
...
def primary_probe_record(record: dict[str, Any]) -> dict[str, Any]:
    ...
    if int(record["signed_mass"]) > 3:
        raise VerificationError("selected primary signed mass exceeds three")
    return {
...
def main() -> int:
    ...
    if candidate.get("schema") != "g0183.six-term-primary-lift-candidate.v1":
    ...
    if tuple(tuple(map(int, term)) for term in left.get("star", [])) != STAR_TERMS:
        raise VerificationError("STAR terms drift")
    if tuple(tuple(map(int, term)) for term in right.get("primary", [])) != PRIMARY_TERMS:
        raise VerificationError("primary terms drift")

    with paths["kernel_basis"].open() as stream:
        kernel_header = json.loads(next(stream))
        kernel_column_zero = json.loads(next(stream))
    if kernel_header.get("schema") != "g0189.exact-primitive-left-kernel-basis.v1":
        raise VerificationError("G-0181 exact-kernel schema drift")
    if kernel_header.get("basis_shape") != [5_769, 478]:
        raise VerificationError("G-0181 exact-kernel shape drift")
    if kernel_column_zero.get("basis_column") != 0:
        raise VerificationError("G-0181 basis column zero missing")
    basis_terms = tuple(
        (int(term[1]), 10 * int(term[2])) for term in kernel_column_zero.get("terms", [])
    )
    if basis_terms != STAR_TERMS:
        raise VerificationError("candidate is not ten times G-0181 basis column zero")
    ...
    with tempfile.TemporaryDirectory(prefix="g0183-independent-replay-") as raw_temp:
    ...
    right_terms = {
        ("primary", sequence): coefficient for sequence, coefficient in PRIMARY_TERMS
    }
    ...
    if len(left_hinges) != 923 or any(direction[0] != 0 for direction in left_hinges):
        raise VerificationError("six-term loopless residual shape drift")
    if (left_union, right_union, residual_union) != (6_002, 3_452, 6_471):
    ...
    receipt = {
        "schema": "g0183.independent-six-term-primary-lift.v1",
        "result": "SECOND_G0181_STAR_KERNEL_RELATION_CERTIFIED_IN_OLD_PRIMARY_SPAN_O",
        "claim_boundary": (
            "Exact complete ordered-chamber normal-form identity for six frozen STAR records "
            "and 21 frozen old-primary records. Together with G-0182 this classifies 2 of "
            "478 G-0181 basis vectors; it does not classify the other 476, prove the full "
            "STAR quarantine, decide MAX11 membership, or establish an unrestricted lower bound."
        ),
        "identity": {
            "left_star": {str(sequence): coefficient for sequence, coefficient in STAR_TERMS},
            "right_primary": {
                str(sequence): coefficient for sequence, coefficient in PRIMARY_TERMS
            },
        },
        "bindings": {
        ...
        "probe_input": {
            ...
            "controlled_records": [
                {"family": family, "sequence": sequence}
                for family, sequence, _ in controlled
            ],
        },
        ...
        "hostile_control": {
            "mutation": "right-side coefficient of p1336 changed from -20 to -19",
